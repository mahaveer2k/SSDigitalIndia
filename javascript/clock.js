...
class PolarGantt {
  ...
  init(opt) {
    ...
    this.hoursScale = d3.scale
      .linear()
      .range([0, 330])
      .domain([0, 23]);
    this.minutesScale = this.secondsScale = d3.scale
      .linear()
      .range([0, 354])
      .domain([0, 59]);
  ...
  _draw(container) {
    ...
    let svg = container.append("svg").attr({
      width: this.width,
      height: this.height
    });

    // Lower Container
    let face = svg.append("g").attr({
      id: "clock-face",
      transform: [
        "translate(",
    ...
    face
      ...
      .attr({
        ...
        y: (d, i) => {
          return -this.hoursLabelRadius * Math.cos(this.radians * 90);
        }
      })
      .text(d => d);

    let field = face
      .selectAll("g.agenda-arc")
      .data(this._fields)
      .enter()
      .append("g")
    ...
    hands.append("line").attr({
      class: "hours-hand",
      x1: 0,
      y1: 0,
      x2: 0,
      y2: this.hoursData.length,
    ...
    hands.append("line").attr({
      ...
      y1: 0,
      x2: 0,
      y2: this.minutesData.length,
      transform:
        "rotate(" + this.minutesData.scale(this.minutesData.value) + ")"
    });
    ...
    hands.append("line").attr({
      ...
      y1: 0,
      x2: 0,
      y2: this.secondsData.length,
      transform:
        "rotate(" + this.secondsData.scale(this.secondsData.value) + ")"
    });
  }
  ...
  _moveHands() {
    d3
      .select("#clock-hands .hours-hand")
      .transition()
      .ease("elastic")
      .duration(1000)
      .attr(
        "transform",
        "rotate(" + this.hoursData.scale(this.hoursData.value) + ")"
      );

    d3
      .select("#clock-hands .minutes-hand")
      .transition()
      .ease("elastic")
      .duration(1000)
      .attr(
    ...
    d3
      .select("#clock-hands .seconds-hand")
      .transition()
      .ease("linear")
      .duration(1000)
      .attr(
